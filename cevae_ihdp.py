...
from utils import fc_net, get_y0_y1, matching_estimate
...
estimation_type = 'approx_posterior'  # /'proxy_matching' / 'latent_matching' / 'approx_posterior'

model_type = 'separated'  # /'standard' / 'separated'

d = 40  # latent dimension for model_type == 'standard'

# for model_type == 'separated':
z_y_dim = d // 2
z_t_dim = d - d // 2
...
for i, (train, valid, test, contfeats, binfeats) in enumerate(dataset.get_train_valid_test()):
    ...
    with tf.Graph().as_default():
        ...
        # -------------------------------------------------------------------------------------------------------------
            ...
            # p(t|zt)
            logits = fc_net(zt, [h], [[1, None]], 'pt_z', lamba=lamba, activation=activation)
            t = Bernoulli(logits=logits, dtype=tf.float32)

            # p(y|t,z)
            mu2_t0 = fc_net(zy, nh * [h], [[1, None]], 'py_t0z', lamba=lamba, activation=activation)
            mu2_t1 = fc_net(zy, nh * [h], [[1, None]], 'py_t1z', lamba=lamba, activation=activation)
            y = Normal(loc=t * mu2_t1 + (1. - t) * mu2_t0, scale=tf.ones_like(mu2_t0))
            ...
            hqz = fc_net(inpt2, (nh - 1) * [h], [], 'qz_xty_shared', lamba=lamba, activation=activation)
            # q(zt|x,t,y)
            muq_t0, sigmaq_t0 = fc_net(hqz, [h], [[z_t_dim, None], [z_t_dim, tf.nn.softplus]], 'qzt_xt0', lamba=lamba,
                                       activation=activation)
            muq_t1, sigmaq_t1 = fc_net(hqz, [h], [[z_t_dim, None], [z_t_dim, tf.nn.softplus]], 'qzt_xt1', lamba=lamba,
                                       activation=activation)
            qzt= Normal(loc=qt * muq_t1 + (1. - qt) * muq_t0, scale=qt * sigmaq_t1 + (1. - qt) * sigmaq_t0)
            # q(zy|x,t,y)
            muq_t0, sigmaq_t0 = fc_net(hqz, [h], [[z_y_dim, None], [z_y_dim, tf.nn.softplus]], 'qzy_xt0', lamba=lamba,
                                       activation=activation)
            muq_t1, sigmaq_t1 = fc_net(hqz, [h], [[z_y_dim, None], [z_y_dim, tf.nn.softplus]], 'qzy_xt1', lamba=lamba,
                                       activation=activation)
            qzy = Normal(loc=qt * muq_t1 + (1. - qt) * muq_t0, scale=qt * sigmaq_t1 + (1. - qt) * sigmaq_t0)

            # Create data dictionary for edward
            data = {x1: x_ph_bin, x2: x_ph_cont, y: y_ph, qt: t_ph, t: t_ph, qy: y_ph}

            # sample posterior predictive for p(y|z,t)
            y_post = ed.copy(y, {zt: qzt, zy: qzy, t: t_ph}, scope='y_post')
            # crude approximation of the above
            y_post_mean = ed.copy(y, {zt: qzt.mean(), zy: qzy.mean(), t: t_ph}, scope='y_post_mean')
            # construct a deterministic version (i.e. use the mean of the approximate posterior) of the lower bound
            # for early stopping according to a validation set
            y_post_eval = ed.copy(y, {zt: qzt.mean(), zy: qzy.mean(), qt: t_ph, qy: y_ph, t: t_ph}, scope='y_post_eval')
            x1_post_eval = ed.copy(x1, {zt: qzt.mean(), zy: qzy.mean(), qt: t_ph, qy: y_ph}, scope='x1_post_eval')
            x2_post_eval = ed.copy(x2, {zt: qzt.mean(), zy: qzy.mean(), qt: t_ph, qy: y_ph}, scope='x2_post_eval')
            t_post_eval = ed.copy(t, {zt: qzt.mean(), zy: qzy.mean(), qt: t_ph, qy: y_ph}, scope='t_post_eval')
            logp_valid = tf.reduce_mean(tf.reduce_sum(y_post_eval.log_prob(y_ph) + t_post_eval.log_prob(t_ph), axis=1) +
                                        tf.reduce_sum(x1_post_eval.log_prob(x_ph_bin), axis=1) +
                                        tf.reduce_sum(x2_post_eval.log_prob(x_ph_cont), axis=1) +
                                        tf.reduce_sum(zt.log_prob(qzt.mean()) - qzt.log_prob(qzt.mean()), axis=1) +
                                        tf.reduce_sum(zy.log_prob(qzy.mean()) - qzy.log_prob(qzy.mean()), axis=1))

            zt_learned = ed.copy(qzt, {x1: x_ph_bin, x2: x_ph_cont})
            zy_learned = ed.copy(qzy, {x1: x_ph_bin, x2: x_ph_cont})
            z_learned = zy_learned  # for matching

            inference = ed.KLqp({zt: qzt, zy: qzy}, data)
        # -------------------------------------------------------------------------------------------------------------
        else:
        ...
        if estimation_type == 'approx_posterior':
            #  estimate potential outcomes based on the inference network output:
        ...
        elif estimation_type == 'proxy_matching':
            #  estimate potential outcomes based on matching the proxy variables
            # training + validation samples:
            y0, y1 = matching_estimate(xalltr, talltr, yalltr, xalltr, n_neighbours=5)
            # for test samples:
            y0t, y1t = matching_estimate(xalltr, talltr, yalltr, xte, n_neighbours=5)

        elif estimation_type == 'latent_matching':
            # estimate potential outcomes based on matching the latent variables
            # L = 100
            # y0, y1 = np.zeros(yalltr.shape, dtype=np.float32), np.zeros(yalltr.shape, dtype=np.float32)
            # for l in range(L):
            #     y0 += sess.run(ymean, feed_dict=f0) / L
            #     y1 += sess.run(ymean, feed_dict=f1) / L
            z_est_alltr = sess.run(z_learned.mean(),
                                   feed_dict={x_ph_bin: xalltr[:, 0:len(binfeats)], x_ph_cont: xalltr[:, len(binfeats):]})

            z_est_test = sess.run(z_learned.mean(),
                                  feed_dict={x_ph_bin: xte[:, 0:len(binfeats)], x_ph_cont: xte[:, len(binfeats):]})

            # training + validation samples:
            y0, y1 = matching_estimate(z_est_alltr, talltr, yalltr, z_est_alltr, n_neighbours=5)
            # for test samples:
            y0t, y1t = matching_estimate(z_est_alltr, talltr, yalltr, z_est_test, n_neighbours=5)

            # TODO: average over sampling of z
